...
if ($ENABLE_CHANGELOG_GENERATOR_MODULE -and $ENABLE_MODPACK_UPLOADER_MODULE) {
    if (!(Test-Path ChangelogGenerator.jar) -or $ENABLE_ALWAYS_UPDATE_JARS) {
        ...
        curl https://github.com/Gaz492/twitch-export-builder/releases/download/1.5.1/twitch-export-builder_linux_amd64 -JLo ./tools/ChangelogGenerator.jar
    }
    ...
    Write-Host "######################################" -ForegroundColor Cyan
    Write-Host ""
    Move-Item "./tools/ChangelogGenerator.jar" . -Force -ErrorAction SilentlyContinue
    java -jar ChangelogGenerator.jar
    Remove-Item MOD_CHANGELOGS.txt -Force -ErrorAction SilentlyContinue
    Remove-Item "./ChangelogGenerator.jar" -Force -ErrorAction SilentlyContinue
...
if ($ENABLE_SERVER_FILE_MODULE -and $ENABLE_MODPACK_UPLOADER_MODULE) {
    ...
    Copy-Item server\* temp -Recurse -Force -ErrorAction SilentlyContinue
    Copy-Item $CONTENTS_TO_ZIP_SERVER temp -Recurse -Force -ErrorAction SilentlyContinue
    Set-Location "./temp"
    7z a -tzip "Server.zip" ./*
